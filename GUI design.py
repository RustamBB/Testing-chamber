...
class ArduinoGUI:
    def __init__(self, master):
        ...
        self.connect_button = ctk.CTkButton(self.port_frame, text="Connect", command=self.connect_arduino, font=('Helvetica', 12,'bold'), fg_color='#007bff', text_color='white')
        self.connect_button.pack(side='left', padx=10)

        self.disconnect_button = ctk.CTkButton(self.port_frame, text="Disconnect", command=self.disconnect_arduino, font=('Helvetica', 12,'bold'), fg_color='#007bff', text_color='white')
        ...
        self.send_button = ctk.CTkButton(self.scale_frame, text="Send", command=self.send_scale_values, font=('Helvetica', 12,'bold'), fg_color='#007bff', text_color='white')
        ...
        self.color_theme_label = ctk.CTkLabel(self.color_theme_frame, text="Select Theme:", font=('Helvetica', 12,'bold'))
        self.color_theme_label.pack(side='left', padx=10)

        self.color_theme_var = ctk.StringVar()
        self.color_theme_menu = ctk.CTkOptionMenu(self.color_theme_frame, values=["Light", "Dark"], variable=self.color_theme_var)
        ...
        if self.ser:
            ...
            if line:
                ...
                for i, value in enumerate(data_values):
                    ...
                    self.axs[row, col].set_xlim(max(0, len(self.x_data[i])-100), len(self.x_data[i]))  # Set x-axis limits
                self.canvas.draw()
